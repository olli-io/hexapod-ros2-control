...
- [`hexa_description/config/gestures.yaml`](../src/hexa_description/config/gestures.yaml) — gestures: keyframed leg + body motions played from a stand on `/cmd_gesture`. A keyframe is one mapping keyed by its type: `pose` carries the values and the `ease` or `continuous` transition (leg poses as joint angles `coxa_deg`, `femur_deg`, `tibia_deg` in geometry.yaml's joint sense, body poses as offsets from the standing pose); `hold` keeps a track where it is, `start` (legs only) delays a leg's take-over so it follows the body until then, and `home` ends a track back at the stance; those three take only `t`. A leg's `start` and `home` are the standing leg under the body pose of that moment, so the ease out of and back into the stand rides the body. Written for the default preset's stance; every leg pose is checked against the joint limits and the ground plane when the pipeline loads.
