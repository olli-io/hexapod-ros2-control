...
- [`hexa_description/config/gestures.yaml`](../src/hexa_description/config/gestures.yaml) — gestures: keyframed leg + body motions played from a stand on `/cmd_gesture`. Leg keyframes in leg-polar coordinates (`angle_deg`, `reach`, `height`), body keyframes as pose offsets, each with an `ease` or `continuous` transition; a `preserve` holds a track. Written for the default preset's stance; validated through IK when the pipeline loads.
