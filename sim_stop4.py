"""Trace disengagement queue when cmd_vel snaps to 0 instantly (no limiter)."""
...
# Walk at 0.3 for 5s, then SNAP cmd_vel to 0 (no limiter)
target_v = (0.30, 0.0); target_w = 0.0
release_t = 5.0
for _ in range(int(12.0 / dt)):
    if t >= release_t:
        target_v = (0.0, 0.0); target_w = 0.0
    vx, vy, w = target_v[0], target_v[1], target_w  # NO LIMITER
    ...
    if just_lifted:
        ...
        s = set(just_lifted)
        label = "TRIPOD A" if s == ta else ("TRIPOD B" if s == tb else str(sorted(s)))
