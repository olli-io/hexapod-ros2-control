...
`pico-2-firmware/src/pipeline.hpp`). The only thing that differs between the Pico
...
Inside the dev container (`./hexa dev`):
...
is exercised off-target by the host harness (`pico-2-firmware/test/host`) — the
