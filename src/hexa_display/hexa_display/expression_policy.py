...
Gaze: during gait-active motion it tracks cmd_vel (forward → UP,
left → LEFT per REP-103); in pose mode it follows body tilt; DEAD
always forces CENTER. Axis quantization uses enter/exit hysteresis so
the gaze doesn't chatter at the deadband edge.
...
def _decide_gaze(
    ...
    if not _cmd_vel_is_zero(inputs):
        # gait-active: gaze leads in the direction of travel.
        vertical = _norm(inputs.vx, config.gaze_vx_max)
        horizontal = _norm(inputs.vy, config.gaze_vy_max) + (
            config.gaze_wz_weight * _norm(inputs.wz, config.gaze_wz_max)
        )
        sv = quantize_axis(
            vertical, prev_v, config.gaze_deadband, config.gaze_exit_ratio
        )
        sh = quantize_axis(
            horizontal, prev_h, config.gaze_deadband, config.gaze_exit_ratio
        )
        return _GAZE_TABLE[(sv, sh)]
    # pose mode: gaze follows body tilt. REP-103 right-handed angles:
    # +pitch (about +y) tips the nose down → gaze DOWN; +yaw turns the
