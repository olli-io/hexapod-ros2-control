...
this file is for running the C++ node in isolation on a bench, loading the
package's own posture.yaml so the animation stack matches production.
"""
...
def generate_launch_description() -> LaunchDescription:
    config = os.path.join(
        get_package_share_directory("hexa_posture_cpp"), "config", "posture.yaml"
    )
