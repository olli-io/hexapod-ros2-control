...
- `msg/GaitParams.msg` — gait selection and the commanded body velocity (`linear_x` / `linear_y` / `angular_z`). Walk-cycle knobs (cycle_time, duty_factor, step_height, stride_length) live in `hexa_gait/config/gait.yaml`, not on the wire.
