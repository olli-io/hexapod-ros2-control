...
- The per-joint default at-rest angle (in the same intuitive degree units) is no longer a file here — it lives in `config/tuning.yaml`'s `gait_node` `standing_pose` ros params, read by the gait node. Decoupled from the servo center so an asymmetric build can set them independently.
