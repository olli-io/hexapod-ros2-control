...
two reference mounts ``l_front`` and ``l_middle``) into the full
six-leg dict by the URDF symmetry rules:
...
to ros2_control without per-joint offsets. If the URDF reference pose is
ever shifted (e.g. so servo zero sits mid-range), add an offset map here.
"""
...
def load_leg_specs(geometry_yaml_path: str | Path) -> dict[str, LegSpec]:
    ...
    for side in ("l", "r"):
        for name in ("front", "middle", "rear"):
            ...
            yaw_fr = math.pi - ref["yaw"] if name == "rear" else ref["yaw"]
            mx = x_fr
