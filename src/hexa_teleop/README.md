...
- **D-pad left / right** — cycles the active gait through `gait_cycle` (`wave → ripple → tripod` by default). D-right advances toward the more dynamic end of the list, D-left toward the more stable end. The switch is only published to `/cmd_gait` when the gait engine reports `stand` (`/gait/state`); presses mid-walk advance the local cursor but no switch lands on the wire until the engine returns to STAND. Works in both posture and gait modes.
