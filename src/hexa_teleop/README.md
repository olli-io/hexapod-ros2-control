...
- **D-pad left / right (gait, posture)** — `gait_prev` / `gait_next` cycle the active gait through `gait_cycle` (`ripple → crawl → tetrapod → surf → tripod` by default). The switch is published to `/cmd_gait` when the gait engine reports `stand`, `gait`, `pausing`, `paused`, or `reseating`. A switch while standing applies immediately; a switch while walking makes the engine pause, reseat with a short dwell, commit the new gait, and resume walking if the stick is still held — further presses mid-sequence keep updating the pending gait. During `engaging` / `resuming` the gait is locked: the press advances the local cursor but the switch is dropped, not queued.
