...
// would otherwise stay in the node's action set until the input watchdog cleared
// it. Long enough that the 60 Hz tick cannot miss the edge; short enough that the
...
function PresetRoute() {
  ...
  return (
    <div id="preset-view">
      {/* The same three modes the Control view's column offers, from the same
          table, so the two can never disagree about which functions exist. Here
          because the animation row below is inert outside animation mode, and
          the way into that mode was on the other view. ANIM is locked wherever
          the robot cannot be in it — on four legs, and on a six-leg preset the
          animations are not written for — and the way out of that is the preset
          grid right underneath. */}
      <h2 className="section-heading" data-section="mode">
        MODE
      </h2>
      <div id="preset-modes">
        {MODES.map((slot) => (
          <button
            key={slot.action}
            className={["preset-mode", slot.mode === state.mode && "active"]
              .filter(Boolean)
              .join(" ")}
            data-mode={slot.mode}
            disabled={
        ...
        {/* One button for both halves of the same press: `init` stands the
            robot from the belly and folds it from a stand, so a second button
            would be a second name for one action — and the label is read off
            /gait/state, never off what was last pressed, so it cannot claim a
            stand the robot did not make. In the mode group, on its own row
            across all three slots: it is the press the modes underneath are
            worth nothing without, and on the belly — where the preset grid
            underneath is dimmed — it is the view's one live action. */}
        <button
          id="preset-stand"
        ...
        >
          <span className="preset-label">{folded ? "STAND" : "FOLD"}</span>
        </button>
      </div>

      {/* Live only from a stand, dimmed everywhere else rather than hidden: a
          preset change re-plants every foot — and moves the middle pair where
          the two presets differ in leg set — so it is a stand-only move. The
          grid is what the view is for, though, and one that came and went would
          move the gait row under a thumb already reaching for it; dimmed, it
          still reads as which preset the robot is on. */}
      <h2 className="section-heading" data-section="preset">
        PRESET
      </h2>
      <div id="preset-list">
        {state.presets.map((preset) => (
          <button
            ...
            // And everything but the animation preset is inert in animation
            // mode, which pins the robot to it.
            disabled={!standing || pending !== null || presetLocked(preset.id)}
            onClick={() => {
              buzz(15);
      ...
      {/* Every declared gait, with the ones the preset in force does not walk
          dimmed in place. Live on the belly too: no gait is running there, but
          /cmd_gait is latched, so the one lit is the one the next stand will
          come up walking — which is worth being able to pick before standing
          rather than after. Also inert while a preset change is in flight, which
          has already published the new preset's entry gait. */}
      {allGaits.length > 0 && (
        <>
          <h2 className="section-heading" data-section="gait">
            GAIT
          </h2>
          <div id="gait-row">
            {allGaits.map((gait) => (
      ...
      {/* The animation rotation, under the gaits and read the same way: the one
          lit is the one latched on /animation/mode. Dimmed rather than hidden
          outside animation mode — nothing is driving the body there, so there is
          nothing to select, but a row that came and went would move the gaits
          under a thumb already reaching for them. The node refuses a selection
          from another mode anyway; this is the same answer, given earlier. */}
      {state.animations.length > 0 && (
        <>
          <h2 className="section-heading" data-section="anim">
            ANIM
          </h2>
          <div id="anim-row">
      ...
      {/* Every tile on the view is already inert while a change is in flight,
          so the modal takes nothing away — it says why, in one place, instead of
          leaving the operator to find the word under one dashed tile. Up only
          for the couple of seconds the move takes: the node clears the pending
          preset on the engine's report, and expires it on its own deadline if
          none arrives. Written here, where the state that raises it lives, but
          rendered above every view — this box is a grid in landscape, which
          sizes a fixed child to a grid area instead of the screen. */}
      {pending !== null && (
